...
*/
static LZ77_compressed* createLiteralLZ77(const uint8_t byte) {
    LZ77_compressed* pLZ77_compressed = (LZ77_compressed*) malloc(sizeof(LZ77_compressed));
    if (pLZ77_compressed == NULL) {
        exit(-1);
    }

    pLZ77_compressed->type = LITERAL;
    pLZ77_compressed->data.literal = byte;

    return pLZ77_compressed;
}
...
* @return LZ77_compressed* The location in memory. MUST BE FREED afterward!
*/
static LZ77_compressed* createMatchLZ77(const uint16_t distance, const uint16_t length) {
    LZ77_compressed* pLZ77_compressed = (LZ77_compressed*) malloc(sizeof(LZ77_compressed));
    if (pLZ77_compressed == NULL) {
        exit(-1);
    }

    pLZ77_compressed->type = MATCH;
    pLZ77_compressed->data.match.distance = distance;
    pLZ77_compressed->data.match.length = length;
 ...
 */
static LZ77_buffer* initLZ77Buffer(void) {
 ...
 */
static void expandBuffer(LZ77_buffer* buffer) {
 ...
 */
static void appendToken(LZ77_buffer* buffer, const LZ77_compressed token) {
 ...
 */
static void freeLZ77Buffer(LZ77_buffer* buffer) {
